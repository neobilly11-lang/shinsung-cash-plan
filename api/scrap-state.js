...
async function readRow() {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/${TABLE}?id=eq.${ROW_ID}&select=scrap_payload,scrap_revision`, {
    headers,
    cache: 'no-store'
  });
  ...
  return { payload: rows[0].scrap_payload || emptyState, revision: rows[0].scrap_revision || 0 };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      const response = await fetch(`${SUPABASE_URL}/rest/v1/${TABLE}?id=eq.${ROW_ID}&scrap_revision=eq.${baseRevision}`, {
        method: 'PATCH',
        headers: { ...headers, Prefer: 'return=minimal' },
        body: JSON.stringify({
          scrap_payload: req.body?.payload || emptyState,
          scrap_revision: nextRevision,
