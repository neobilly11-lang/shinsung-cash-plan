(function fieldRequiredFlowV1(){
  ...
  function wrapAlert(){
    ...
    function wrapped(message){
      const result=original.apply(this,arguments);
      const msgText=String(message==null?'':message);
      if(ACTIONABLE_ERROR.test(msgText)&&!NON_FIELD_ERROR.test(msgText))queueMicrotask(()=>guideFromError('',msgText));
      return result;
    }
    wrapped.__requiredFlowWrapped=true;wrapped.__original=original;window.alert=wrapped;return true;
  }
  function isSaveAction(button){
    if(!button)return false;
    const text=norm(button.textContent||button.value),handler=actionHandler(button);
    const saveHandler=/^(save|confirm|create|addPackage|addDomesticPackage)/i.test(handler);
    if(IGNORE_ACTION.test(text)&&!saveHandler)return false;
    return SAVE_WORDS.test(text)||saveHandler;
  }
