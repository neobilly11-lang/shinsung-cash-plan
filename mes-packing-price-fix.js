(function mesPackingPriceFixV1(){
  ...
  window.showMesDetailEditor=function(){
    ...
    if(['purchase','settlement'].includes(view)){
      var rows=row.rows||safe(state.pos).filter(function(x){return x.poNo===row.poNo;}),source=rows[0]||{};
      $('modalBody').innerHTML='<form class="form-grid" onsubmit="saveMesDetail(event,this)"><label>P.O 번호<input name="poNo" value="'+esc(row.poNo)+'"></label><label>거래처<input name="company" value="'+esc(row.company)+'"></label><label>입고예정일<input name="expected" type="date" value="'+(date(row.expected)==='-'?'':date(row.expected))+'"></label><label>SHIPMENTS<input name="shipment" value="'+esc(source.shipment||source.shippingTerm||source.loadingTerm||'')+'"></label><label>PAYMENT<input name="paymentTerm" value="'+esc(source.paymentTerm||source.payment||source.paymentTerms||'')+'"></label><label>통화<select name="currency"><option '+(row.currency==='KRW'?'selected':'')+'>KRW</option><option '+(row.currency!=='KRW'?'selected':'')+'>USD</option></select></label><label>환율<input name="rate" type="number" step="0.01" value="'+(num(row.rate)||1)+'"></label><label>세틀상태<select name="settlementStatus"><option>미완료</option><option '+(row.rows&&row.rows[0]&&row.rows[0].settlementStatus==='검토중'?'selected':'')+'>검토중</option><option '+(row.rows&&row.rows[0]&&row.rows[0].settlementStatus==='완료'?'selected':'')+'>완료</option></select></label><div class="registration-lines"><h3>사내입고 품목 · 단가 수정 가능</h3>'+rows.map(function(x){return mesEditLine(x,'PO');}).join('')+'</div><div class="wide actions"><button class="btn primary">환율·단가·SHIPMENTS·PAYMENT 수정 저장</button><button type="button" class="btn" onclick="openMesDetail(\''+view+'\',\''+esc(id)+'\')">취소</button></div></form>';
      return;
    }
  ...
  window.saveMesDetail=async function(event,form){
    ...
    event.preventDefault();var v=formData(form),ok=await commit('P.O 상세 품종·환율·단가·SHIPMENTS·PAYMENT 수정',['pos'],function(s){form.querySelectorAll('.edit-line').forEach(function(line){var x=s.pos.find(function(p){return p.id===line.dataset.id;});if(!x)return;var d=mesLineData(line),unitPrice=round2(d.unitPrice),weight=round2(d.weight),packingLinked=!!(x.inboundRequestId||x.inboundRequestNo||x.packingPackageNo);Object.assign(x,{poNo:v.poNo,company:v.company,expectedArrivalDate:v.expected,shipment:v.shipment,shippingTerm:v.shipment,loadingTerm:v.shipment,paymentTerm:v.paymentTerm,payment:v.paymentTerm,paymentTerms:v.paymentTerm,currency:v.currency,exchangeRate:num(v.rate)||1,productType:d.productType,purchaseContractGrade:d.grade,contractGrade:d.grade,purchaseContractWeight:weight,contractWeight:weight,unitPrice:unitPrice,purchaseUnitPrice:unitPrice,amount:round2(weight*unitPrice),receiptStatus:d.receiptStatus,inspectionStatus:d.inspectionStatus,settlementStatus:v.settlementStatus,updatedAt:new Date().toISOString(),updatedByName:currentUserName()});if(!packingLinked)Object.assign(x,{grade:d.grade,weight:weight,grossWeight:weight,netWeight:weight});});});
    if(ok){openMesDetail(view,v.poNo);toast('품종·환율과 품목별 단가까지 수정 저장했습니다.');}
  };
  window.mesPurchaseDetail=function(row){
    ...
    return mesSection('구매 계약 세부내역',[['P.O',function(x){return x.poNo;}],['거래처',function(x){return x.company;}],['품종',function(x){return x.productType||'-';}],['거래처 강종',function(x){return x.purchaseContractGrade||x.contractGrade||x.grade;}],['패키지',function(x){return x.packageNo;}],['계약중량',function(x){return fmt(x.purchaseContractWeight||x.contractWeight||x.weight);}],['단가('+esc(row.currency||'USD')+'/kg)',function(x){return fmt(x.unitPrice||x.purchaseUnitPrice);}],['통화',function(x){return x.currency||row.currency;}],['환율',function(x){return fmt(x.exchangeRate||row.rate);}],['금액',function(x){var w=num(x.purchaseContractWeight||x.contractWeight||x.weight);return fmt(x.amount||w*num(x.unitPrice||x.purchaseUnitPrice));}],['입고예정일',function(x){return date(x.expectedArrivalDate||x.arrivalDate);}],['구매상태',function(x){return x.purchaseStatus||x.status;}]],rows);
  };
