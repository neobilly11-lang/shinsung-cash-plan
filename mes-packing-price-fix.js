(function mesPackingPriceFixV1(){
  ...
  window.saveMesDetail=async function(event,form){
    ...
    event.preventDefault();var v=formData(form),ok=await commit('P.O 상세 환율·단가·SHIPMENTS·PAYMENT 수정',['pos'],function(s){form.querySelectorAll('.edit-line').forEach(function(line){var x=s.pos.find(function(p){return p.id===line.dataset.id;});if(!x)return;var d=mesLineData(line),unitPrice=round2(d.unitPrice),weight=round2(d.weight),packingLinked=!!(x.inboundRequestId||x.inboundRequestNo||x.packingPackageNo);Object.assign(x,{poNo:v.poNo,company:v.company,expectedArrivalDate:v.expected,shipment:v.shipment,shippingTerm:v.shipment,loadingTerm:v.shipment,paymentTerm:v.paymentTerm,payment:v.paymentTerm,paymentTerms:v.paymentTerm,currency:v.currency,exchangeRate:num(v.rate)||1,purchaseContractGrade:d.grade,contractGrade:d.grade,purchaseContractWeight:weight,contractWeight:weight,unitPrice:unitPrice,purchaseUnitPrice:unitPrice,amount:round2(weight*unitPrice),receiptStatus:d.receiptStatus,inspectionStatus:d.inspectionStatus,settlementStatus:v.settlementStatus,updatedAt:new Date().toISOString(),updatedByName:currentUserName()});if(!packingLinked)Object.assign(x,{grade:d.grade,weight:weight,grossWeight:weight,netWeight:weight});});});
