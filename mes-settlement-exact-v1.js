(function(){
  'use strict';
  var VERSION='20260820-template-fit-v5';
  ...
  function grade(row){return [row&&row.productType,row&&(row.mainGrade||row.grade),row&&row.subGrade,row&&row.detailGrade].filter(Boolean).join(' · ')}
  ...
  function exactDetail(poNo){
    ...
    var originalRows=packages.map(function(row){var weight=num(row.netWeight||row.nw||row.weight),price=priceOf(row);return{description:row.grade||grade(row)||'미지정',weight:weight,price:price,amount:weight*price}});
    var actualRows=splits.map(function(row){var source=packageMap.get(String(row.packageNo||''))||{},price=priceOf(row)||priceOf(source),remark=photoMemo(row,'anomaly')||photoMemo(row,'shape')||row.memo||'';return{description:grade(row)||row.grade||source.grade||'미지정',weight:num(row.weight),price:price,amount:num(row.weight)*price,remark:remark,hasAnomaly:hasAnomaly(row),loss:false}});
    losses.forEach(function(row){var source=packageMap.get(String(row.packageNo||''))||{},price=priceOf(source);actualRows.push({description:'LOSS'+(source.grade?' ('+source.grade+')':''),weight:num(row.weight),price:price,amount:0,remark:row.reason||row.memo||'LOSS',hasAnomaly:hasAnomaly(row),loss:true})});
    if(!actualRows.length){actualRows=packages.map(function(row){return{description:row.grade||'검수 미완료',weight:0,price:priceOf(row),amount:0,remark:'',hasAnomaly:false,loss:false}})}
  ...
  function fillOverseas(doc,detail,sheetName){
    ...
    setCell(doc,'B5',detail.company);setCell(doc,'B6',lines[0]);setCell(doc,'B7',lines[1]);setCell(doc,'B8',[detail.phone,detail.fax].filter(Boolean).join('   '));setCell(doc,'G5',dateSerial,'number');setCell(doc,'G6',detail.invoiceNo);setCell(doc,'G7',detail.poNo);clearRange(doc,0,9,start,totalRow-1);setSmall(doc,'J12','Photo',6.5);
    ...
    actual.forEach(function(row,index){var r=start+index;setCell(doc,'E'+r,row.description);setCell(doc,'F'+r,row.weight,'number');setCell(doc,'G'+r,raw?'':(row.price||''),!raw&&row.price?'number':'string');setCell(doc,'H'+r,raw?0:(row.amount||0),'number');setSmall(doc,'I'+r,row.remark||'',6.5);setCell(doc,'J'+r,row.hasAnomaly?'●':'')});
  ...
  function injectStyles(){if(document.getElementById('mesSettlementExactStyles'))return;var style=document.createElement('style');style.id='mesSettlementExactStyles';style.textContent=`
    .settlement-excel-preview-shell{width:100%;overflow:hidden;background:#dfe3e2;padding:6px;box-sizing:border-box;position:relative}.settlement-excel-preview-shell *{box-sizing:border-box}.settlement-excel-paper{margin:0;background:#fff;color:#111;font-family:Arial,"Malgun Gothic",sans-serif;box-shadow:0 2px 12px #0002;overflow:hidden}.settlement-excel-paper table{border-collapse:collapse;table-layout:fixed;width:100%;margin:0}.settlement-excel-paper th,.settlement-excel-paper td{border:1px solid #5b5b5b!important;background:#fff!important;color:#111;font-weight:400;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.settlement-excel-paper .num{text-align:right}
  ...
  window.addEventListener('resize',scheduleFit,{passive:true});injectStyles();
})();
