...
function fitPreview(){document.querySelectorAll('.settlement-excel-preview-shell').forEach(function(shell){var paper=shell.querySelector('.settlement-excel-paper');if(!paper)return;var base=paper.classList.contains('settlement-domestic-paper')?1180:760;shell.style.height='auto';shell.style.minHeight='0';paper.style.width=base+'px';paper.style.transform='none';paper.style.margin='0';var available=Math.max(1,shell.clientWidth-12);if(available<10)return;var naturalHeight=Math.max(1,paper.scrollHeight),scale=Math.min(1,available/base),visualWidth=base*scale;paper.style.transformOrigin='top left';paper.style.transform='scale('+scale+')';paper.style.marginLeft=Math.max(0,(available-visualWidth)/2)+'px';shell.style.height=Math.ceil(naturalHeight*scale+12)+'px';shell.dataset.previewScale=String(scale);shell.dataset.previewVisualHeight=String(Math.round(naturalHeight*scale))})}
