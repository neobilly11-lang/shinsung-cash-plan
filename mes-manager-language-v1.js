(function(){
  ...
  function sourceGrade(row){return text(row&&row.grade)||[row&&row.productType,row&&(row.mainGrade||row.finalGrade),row&&row.subGrade,row&&row.detailGrade].map(text).filter(Boolean).join(' · ')||'-'}
  function sourceStatusOpen(row){var status=text(row&&row.status).toUpperCase();return !/CANCEL|DELETED|DEPLETED|CLOSED/.test(status)}
  function sourceRowsForTask(task){
    var s=state(),rows=[];
    function push(id,label,weight,meta){weight=number(weight);if(!text(id)||weight<=0)return;rows.push({id:text(id),label:text(label)||text(id),weight:weight,meta:text(meta)})}
    if(task==='INBOUND'){
      var poGroups={};list(s.pos).filter(sourceStatusOpen).forEach(function(row){var po=text(row.poNo)||'미지정 P.O';(poGroups[po]||(poGroups[po]=[])).push(row)});
      Object.keys(poGroups).forEach(function(po){var items=poGroups[po],weight=items.reduce(function(sum,row){return sum+rowWeight(row)},0),company=text(items[0]&&items[0].company),grades=Array.from(new Set(items.map(sourceGrade).filter(function(x){return x&&x!=='-'}))).slice(0,3).join(' / ');push('PO:'+po,po+' · '+company,weight,grades+' · '+items.length+'개 품목')});
    }else if(task==='INSPECTION'){
      var splitBy={},lossBy={};list(s.splits).filter(sourceStatusOpen).forEach(function(row){var key=text(row.packageNo);splitBy[key]=(splitBy[key]||0)+rowWeight(row)});list(s.losses).filter(sourceStatusOpen).forEach(function(row){var key=text(row.packageNo);lossBy[key]=(lossBy[key]||0)+rowWeight(row)});
      list(s.pos).filter(sourceStatusOpen).forEach(function(row){var no=text(row.packageNo||row.internalNo||row.id),status=text(row.inspectionStatus).toUpperCase(),remain=Math.max(0,rowWeight(row)-number(splitBy[no])-number(lossBy[no]));if(/COMPLETE|DONE|FINAL/.test(status)||remain<=0)return;push('PKG:'+no,no+' · '+text(row.company),remain,sourceGrade(row)+' · 검수잔량')});
    }else if(['GENERAL','SHOT','CUTTING','ACID','RESELECT'].indexOf(task)>=0){
      list(s.workWaits).filter(function(row){return sourceStatusOpen(row)&&!finished(row)&&canonicalTask(row,'GENERAL')===task}).forEach(function(row){var no=text(row.packageNo||row.internalNo||row.id),work=text(row.workType||row.type||taskLabel(task));push('WORK:'+text(row.id||no),no+' · '+work,rowWeight(row),sourceGrade(row)+' · '+text(row.location||row.workLocation||row.memo))});
    }else if(task==='SHIPPING'){
      var soGroups={};list(s.salesOrders).filter(sourceStatusOpen).forEach(function(row){var so=text(row.soNo)||text(row.id);(soGroups[so]||(soGroups[so]=[])).push(row)});var shippedBy={};list(s.shipments).filter(sourceStatusOpen).forEach(function(row){var so=text(row.soNo);shippedBy[so]=(shippedBy[so]||0)+rowWeight(row)});
      Object.keys(soGroups).forEach(function(so){var items=soGroups[so],planned=items.reduce(function(sum,row){return sum+rowWeight(row)},0),remaining=Math.max(0,planned-number(shippedBy[so]));if(remaining<=0)return;var grades=Array.from(new Set(items.map(sourceGrade).filter(function(x){return x&&x!=='-'}))).slice(0,3).join(' / ');push('SO:'+so,so+' · '+text(items[0]&&items[0].customer),remaining,grades+' · 출하잔량')});
    }else if(task==='MOVE'){
      list(s.bags).filter(function(row){return sourceStatusOpen(row)&&rowWeight(row)>0}).forEach(function(row){var no=text(row.completionNo||row.bagNo||row.code||row.id);push('BAG:'+text(row.id||no),no,rowWeight(row),sourceGrade(row)+' · '+text(row.location||'위치 미지정'))});
    }
    return rows.sort(function(a,b){return a.label.localeCompare(b.label,'ko')})
  }
  function mergeSavedSources(task,saved){var rows=sourceRowsForTask(task),seen=new Set(rows.map(function(row){return row.id}));list(saved&&saved.sourceSnapshot).forEach(function(row){if(row&&row.id&&!seen.has(text(row.id))){rows.push({id:text(row.id),label:text(row.label)||text(row.id),weight:number(row.weight),meta:text(row.meta)||'저장 당시 자료'});seen.add(text(row.id))}});return rows}
  function sourcePickerHtml(task,saved){var selected=new Set(list(saved&&saved.selectedSourceIds).map(text)),rows=mergeSavedSources(task,saved);return'<div class="manager-source-head"><div><h3>'+esc(language==='en'?'Select actual work records':'실제 작업자료 선택')+'</h3><small>'+esc(language==='en'?'Select multiple records. Weight is summed automatically.':'여러 건을 선택하면 중량이 자동 합산됩니다.')+'</small></div><input type="search" id="productionSourceSearch" placeholder="'+esc(language==='en'?'Search P.O, internal no., S.O, grade':'P.O·사내입고번호·S.O·강종 검색')+'" oninput="filterProductionSources(this.value)"></div><div id="productionSourceList" class="manager-source-list">'+(rows.length?rows.map(function(row){var on=selected.has(row.id);return'<label class="manager-source '+(on?'on':'')+'" data-search="'+esc((row.label+' '+row.meta).toLowerCase())+'"><input type="checkbox" name="sourceIds" value="'+esc(row.id)+'" '+(on?'checked':'')+' onchange="toggleProductionSource(this)"><span><b>'+esc(row.label)+'</b><small>'+esc(row.meta)+'</small></span><strong>'+fmt(row.weight)+' kg</strong></label>'}).join(''):'<p class="empty">'+esc(language==='en'?'No selectable records.':'선택 가능한 실제 자료가 없습니다.')+'</p>')+'</div>'}
  ...
  function assignmentCards(){var rows=assignments().slice().sort(function(a,b){return text(b.createdAt).localeCompare(text(a.createdAt))});if(!rows.length)return'<p class="empty">저장된 작업분배가 없습니다.</p>';return'<div class="manager-assignment-list">'+rows.map(function(a){var sources=list(a.sourceSnapshot);return'<article><div><b>'+esc(taskLabel(a.taskType))+' · '+fmt(a.targetWeight)+' kg</b><small>'+(sources.length?esc(sources.length+'건 · '+sources.slice(0,2).map(function(x){return x.label}).join(' / ')+(sources.length>2?' 외 '+(sources.length-2)+'건':'')):'수기 배분')+'</small><small>'+esc(list(a.userNames).join(' + '))+' · '+fmt(a.capacityPerHour,0)+' kg/h</small><small>'+fmt(a.estimatedHours,2)+'시간 · '+fmt(a.estimatedDays,2)+'일 · '+Math.round(number(a.estimatedCost)).toLocaleString('ko-KR')+'원 · 달성예상 '+fmt(a.achievementRate,0)+'%</small></div><div class="actions"><button class="btn" onclick="editProductionAssignment(\''+esc(a.id)+'\')">수정</button><button class="btn danger" onclick="deleteProductionAssignment(\''+esc(a.id)+'\')">삭제</button></div></article>'}).join('')+'</div>'}
  ...
  root.filterProductionSources=function(query){var q=text(query).toLowerCase();doc.querySelectorAll('#productionSourceList .manager-source').forEach(function(row){row.hidden=!!q&&text(row.dataset.search).indexOf(q)<0})};
  root.toggleProductionSource=function(input){var row=input.closest('.manager-source');if(row)row.classList.toggle('on',input.checked);root.updateProductionAssignmentEstimate()};
  root.refreshProductionSourcePicker=function(){var form=doc.getElementById('productionAssignmentForm'),box=doc.getElementById('productionSourcePicker');if(!form||!box)return;box.innerHTML=sourcePickerHtml(form.elements.taskType.value,{});root.updateProductionAssignmentEstimate()};
  function estimateFromForm(){var form=doc.getElementById('productionAssignmentForm');if(!form)return null;function field(name){return form.elements&&form.elements.namedItem?form.elements.namedItem(name):form.querySelector('[name="'+name+'"]')}var task=field('taskType').value,allRows=sourceRowsForTask(task),checked=Array.from(form.querySelectorAll('input[name="sourceIds"]:checked')),ids=checked.map(function(input){return input.value}),snapshot=checked.map(function(input){var row=allRows.find(function(item){return item.id===input.value}),card=input.closest('.manager-source');return row||{id:input.value,label:text(card&&card.querySelector('b')&&card.querySelector('b').textContent),weight:number(text(card&&card.querySelector('strong')&&card.querySelector('strong').textContent).replace(/[^0-9.+-]/g,'')),meta:text(card&&card.querySelector('small')&&card.querySelector('small').textContent)}}),target=snapshot.reduce(function(sum,row){return sum+number(row.weight)},0);field('targetWeight').value=target?String(target):'0';var names=Array.from(form.querySelectorAll('.manager-user.on')).map(function(b){return b.dataset.user}),caps=capacities(),rates=caps.map(function(u){return{user:u.name,rate:u.by[task]?number(u.by[task].rate):0}}).filter(function(x){return x.rate>0}).sort(function(a,b){return b.rate-a.rate}),recommended=names.length||Math.min(2,Math.max(1,target>0&&rates[0]?Math.ceil(target/(rates[0].rate*8)):1)),selectedRates=names.map(function(name){var u=rates.find(function(x){return x.user===name});return u?u.rate:0}),rate=names.length?selectedRates.reduce(function(sum,value){return sum+value},0):rates.slice(0,recommended).reduce(function(sum,row){return sum+row.rate},0);if(rate<=0){var def=taskDefs.find(function(t){return t.id===task});rate=(def?def.defaults:1)*recommended}var hours=rate>0?target/rate:0,days=hours/8,cost=hours*recommended*number(field('hourlyCost').value),due=field('dueDate').value,dueDays=due?Math.max(1,Math.ceil((new Date(due+'T23:59:59')-new Date())/864e5)):0,achievement=dueDays>0&&days>0?Math.min(100,dueDays/days*100):(rate>0&&target>0?100:0);return{taskType:task,targetWeight:target,selectedSourceIds:ids,sourceSnapshot:snapshot,userNames:names,recommendedPeople:recommended,capacityPerHour:rate,estimatedHours:hours,estimatedDays:days,estimatedCost:cost,dueDate:due,achievementRate:achievement,memo:text(field('memo').value),hourlyCost:number(field('hourlyCost').value)}}
  root.updateProductionAssignmentEstimate=function(){var e=estimateFromForm(),box=doc.getElementById('productionAssignmentEstimate');if(!e||!box)return;box.innerHTML='<div><small>선택 자료</small><b>'+e.selectedSourceIds.length+'건 · '+fmt(e.targetWeight)+' kg</b></div><div><small>예상 인원</small><b>'+e.recommendedPeople+'명</b></div><div><small>합산 생산능력</small><b>'+fmt(e.capacityPerHour,0)+' kg/h</b></div><div><small>예상 작업시간</small><b>'+fmt(e.estimatedHours,2)+'시간</b></div><div><small>예상 작업일수</small><b>'+fmt(e.estimatedDays,2)+'일</b></div><div><small>예상 작업비</small><b>'+Math.round(e.estimatedCost).toLocaleString('ko-KR')+'원</b></div><div><small>달성예상율</small><b>'+fmt(e.achievementRate,0)+'%</b></div>'};
  root.saveProductionAssignment=async function(event,form,id){event.preventDefault();var e=estimateFromForm();if(!e||!e.selectedSourceIds.length||e.targetWeight<=0)return toast('실제 작업자료를 한 건 이상 선택하세요.',true);if(!e.userNames.length)return toast('작업자를 한 명 이상 합류시키세요.',true);var ok=await commit('생산 작업분배 저장',['productionAssignments'],function(s){s.productionAssignments=list(s.productionAssignments);var old=s.productionAssignments.find(function(x){return x.id===id}),record=Object.assign(old||{id:uid(),createdAt:now(),status:'WAITING'},e,{updatedAt:now(),updatedByName:userName()});if(!old)s.productionAssignments.push(record)});if(ok){toast('작업분배를 공용 서버에 저장했습니다.');root.openProductionManager()}};
...
.manager-user-picker{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;margin:10px 0 18px}.manager-user{display:flex;justify-content:space-between;align-items:center;padding:14px 16px;border:2px solid #dfe8eb;border-radius:14px;background:#fff;font:inherit}.manager-user.on{border-color:#07989c;background:#e6f8f5;color:#075a62}.manager-user b{color:#07848a}\
  .manager-source-head{display:flex;justify-content:space-between;align-items:end;gap:14px;margin:18px 0 10px}.manager-source-head h3{margin:0 0 4px}.manager-source-head input{width:min(420px,100%)}.manager-source-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:9px;max-height:340px;overflow:auto;padding:10px;border:1px solid #dbe7e9;border-radius:16px;background:#f7fafb}.manager-source{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:10px;align-items:center;padding:12px;border:2px solid transparent;border-radius:12px;background:#fff;cursor:pointer}.manager-source.on{border-color:#07989c;background:#e9f9f6}.manager-source input{width:22px;height:22px}.manager-source span,.manager-source b,.manager-source small{min-width:0}.manager-source b,.manager-source small{display:block}.manager-source small{margin-top:3px;color:#61717d;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.manager-source strong{white-space:nowrap;color:#075a62}.manager-estimate{display:grid;grid-template-columns:repeat(7,minmax(110px,1fr));gap:10px;padding:14px;background:#f1f6f8;border-radius:16px}.manager-estimate>div{padding:12px;background:#fff;border-radius:12px}.manager-estimate small,.manager-estimate b{display:block}.manager-estimate b{margin-top:6px;font-size:1.15rem;color:#075a62}.manager-note{display:block;margin:16px 0}.manager-note textarea{width:100%;min-height:90px}\
.manager-assignment-list{display:grid;gap:10px}.manager-assignment-list article{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:14px 16px;border:1px solid #dce6e9;border-radius:14px;background:#fff}.manager-assignment-list article>div:first-child{min-width:0}.manager-assignment-list .actions{margin:0;flex-wrap:nowrap}\
  @media(max-width:760px){.manager-banner{align-items:flex-start;flex-direction:column}.manager-head{align-items:stretch}.manager-capacity-table{max-height:55vh}.manager-source-head{align-items:stretch;flex-direction:column}.manager-source-head input{width:100%}.manager-source-list{grid-template-columns:1fr;max-height:48vh}.manager-estimate{grid-template-columns:repeat(2,minmax(0,1fr))}.manager-assignment-list article{align-items:stretch;flex-direction:column}.mes-language-toggle{min-height:44px}}';doc.head.appendChild(style)}
  ...
  root.__mesManagerLanguageV1={get language(){return language},capacities:capacities,events:events,sourceRowsForTask:sourceRowsForTask,translate:translateDOM,version:'20260818-13'};
