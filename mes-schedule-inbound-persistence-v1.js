(function(){
  ...
  var typeFilter={purchase:true,sales:true};
  var query='';
  ...
  function purchaseEvents(){
    ...
    list(poRows()).forEach(function(po){
      var request=requestForPo(po.poNo),when=dateKey(request&&request.requestDate||po.expected),items=request&&request.items||po.rows;
      if(!when)return;
    ...
    list(state.purchaseRequests).forEach(function(request){
      ...
      var when=dateKey(request.requestDate||request.createdAt);if(!when)return;
  ...
  function salesEvents(){
    ...
    return rows.map(function(order){
      ...
      var when=dateKey(request.shippingRequestDate||request.requestDate||order.shipDate);if(!when)return null;
  ...
  function allEvents(){
    ...
    return purchaseEvents().concat(salesEvents()).filter(function(event){
  ...
  function openEvent(event){
    var view=event.type==='purchase'?'purchase':'sales';
    return "openMesDetail('"+view+"',decodeURIComponent('"+encodeURIComponent(event.id)+"'))";
  }
  function eventHtml(event){
    var label=(event.time?event.time+' ':'')+event.partner+' · '+event.grade+' · '+event.number;
    return"<button class='schedule-event "+event.type+"' title='"+escapeHtml(label)+"' onclick=\""+openEvent(event)+"\"><span></span><b>"+escapeHtml(event.time||'')+"</b> "+escapeHtml(short(event.partner))+' · '+escapeHtml(short(event.grade))+"<small>"+escapeHtml(event.number)+"</small></button>";
  }
  ...
  function mobileList(events){
    ...
    if(!rows.length)return"<div class='schedule-empty'>이 달의 구매·판매 일정이 없습니다.</div>";
  ...
  function renderSchedule(){
    ...
    $('content').innerHTML="<div class='schedule-head'><div><h1>입고·출하 일정</h1><p>구매계획 입고일과 판매계획 출하일을 한눈에 확인합니다.</p></div><div class='schedule-search'><input id='mesScheduleQuery' value='"+escapeHtml(query)+"' placeholder='P.O · S.O · 거래처 · 거래처 강종 검색'><button class='btn primary' onclick='mesScheduleSearch()'>검색</button></div></div>"
      +"<div class='schedule-shell'><aside class='schedule-side'><button class='btn primary wide' onclick=\"openView('purchase')\">구매계획 열기</button><button class='btn wide' onclick=\"openView('sales')\">판매계획 열기</button><div class='mini-title'><button onclick='mesScheduleMove(-1)'>‹</button><b>"+escapeHtml(monthTitle(monthCursor))+"</b><button onclick='mesScheduleMove(1)'>›</button></div>"+miniCalendar()+"<div class='schedule-types'><label><input type='checkbox' "+(typeFilter.purchase?'checked':'')+" onchange=\"mesScheduleToggle('purchase',this.checked)\"><i class='purchase'></i> 구매 입고일정</label><label><input type='checkbox' "+(typeFilter.sales?'checked':'')+" onchange=\"mesScheduleToggle('sales',this.checked)\"><i class='sales'></i> 판매 출하일정</label></div></aside>"
