(function(){
  ...
  function containerWorkEvents(){
    var rows=[];try{rows=list(salesRows());}catch(_){rows=[];}
    return rows.map(function(order){
      var items=list(order.items),source=items.find(function(item){return item.shippingConfirmedDate||item.shippingConfirmedAt;})||{};
      var when=dateKey(source.shippingConfirmedDate||source.shippingConfirmedAt||order.shippingConfirmedDate||order.shippingConfirmedAt);if(!when)return null;
      return{type:'containerWork',date:when,time:text(source.shippingConfirmedTime||order.shippingConfirmedTime),number:order.soNo,partner:text(order.customer),grade:fullGrade(items,order.grade),weight:number(order.weight),id:order.soNo};
    }).filter(Boolean);
  }
  ...
  function openEvent(event){
    var view=event.type==='departureConfirmed'?'shipping':(['sales','departurePlan','containerWork'].includes(event.type)?'sales':'purchase');
    return "openMesDetail('"+view+"',decodeURIComponent('"+encodeURIComponent(event.id)+"'))";
  }
  function eventHtml(event){
    var marker=event.time||(event.type==='purchase'?'입고요청':event.type==='arrival'?'입항예정':event.type==='sales'?'출하요청':event.type==='departurePlan'?'출항예정':event.type==='containerWork'?'컨테이너작업':event.type==='departureConfirmed'?'출항확정':'');
